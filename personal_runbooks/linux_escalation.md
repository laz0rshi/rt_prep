...
```sh
lsblk # mounted file system 
lpstat # printers
cat /etc/fstab # mounted drives
cat /etc/fstab | grep -v "#" | column -t # Unmounted
df -h
```
...
```sh
# Users
cat /etc/passwd # check for hashed passwords
cut -d: -f1 /etc/passwd # list of users
sudo --version # if version 1.18.31 exploitable 
grep "*sh$" /etc/passwd 
ls -lah /home
...
- SSH Directory and hidden files
